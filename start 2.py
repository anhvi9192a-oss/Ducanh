...
--------------------------------------""" 
    ...
    v = message.split()
    if len(v) < 2:
        return self._bot.reply(cid, type, "[B][c]Dùng: /all AK47 hoặc /all rd hoặc /all s7 hoặc /all full s7")

    if not self.ids:
        return self._bot.reply(cid, type, "[B][c]Vui Lòng Dùng /js Trước Để Lấy Danh Sách ID")

    cc = v[1].upper()
    ids = list(set(self.ids))  # Danh sách người trong team
    emotes = list(self.Emotes.values())
    
    # --- TRƯỜNG HỢP: /all rd s7 (Random hành động cho toàn bộ team theo đợt) ---
    if cc == "FULL" and len(v) >= 3 and v[2].lower() == "s7":
        emote_list = emotes[:]
        random.shuffle(emote_list)
        
        # Để đảm bảo tất cả mọi người đều nhận được hành động liên tục:
        for emote in emote_list:
            # Gửi hành động đó cho TOÀN BỘ danh sách ids cùng lúc (S7 style)
            # Hoặc chia nhỏ nếu socket bị giới hạn, nhưng ở đây ta dùng toàn bộ ids
            self.sock39699.send(self._bot.play_emote(emote, ids))
            time.sleep(8.5) # Delay tránh spam/lag server
            
        return self._bot.reply(cid, type, "[B][c]EMOTE --> RANDOM S7 FULL DONE")

    # --- TRƯỜNG HỢP: /all s7 (Gửi lần lượt tất cả emote cho cả team) ---
    if cc == "S7":
        for emote in emotes:
            self.sock39699.send(self._bot.play_emote(emote, ids))
            time.sleep(4.5)
        return self._bot.reply(cid, type, "[B][c]EMOTE --> S7 ALL")

    # --- TRƯỜNG HỢP: /all rd (Mỗi người 1 hành động ngẫu nhiên khác nhau) ---
    if cc == "RD":
        for uid in ids:
            emote = random.choice(emotes)
            self.sock39699.send(self._bot.play_emote(emote, [uid]))
            # Lưu ý: Nếu team đông, delay nên thấp hơn một chút để tránh chờ lâu
            time.sleep(1.0) 
        return self._bot.reply(cid, type, "[B][c]EMOTE --> RANDOM")

    # --- TRƯỜNG HỢP: /all [Tên Emote] (Ví dụ: /all AK47) ---
    if cc in self.Emotes:
        self.sock39699.send(self._bot.play_emote(self.Emotes[cc], ids))
        return self._bot.reply(cid, type, f"[B][c]EMOTE --> {cc}")
    
    else:
        available_emotes = ", ".join(self.Emotes.keys())
        return self._bot.reply(cid, type, f"[B][c]Không tìm thấy! Có sẵn:\n{available_emotes}")
    ...
    # Đợi 1.5 giây để server ổn định vị trí bot trong team
    time.sleep(1.5) 
    
    # 2. Mặc Skin (Random hoặc chọn số)
    idlist = {
    ...
    if len(parts) >= 2 and parts[1].isdigit():
        ...
        sid = idlist.get(skin_choice, random.choice(list(idlist.values())))
    else:
        sid = random.choice(list(idlist.values()))

    self.sock39699.send(self._bot.showskin(sid))
    
    # --- ĐOẠN QUAN TRỌNG: CẬP NHẬT LẠI DANH SÁCH ID ---
    # Đợi thêm một chút để nhận packet thông tin team từ server
    time.sleep(2.0) 
    
    # Nếu self.ids vẫn rỗng, chúng ta thử lấy từ biến nội bộ của bot nếu có
    current_ids = list(set(self.ids)) if self.ids else []
    
    if not current_ids:
        # Thông báo nếu vẫn không tìm thấy ai để múa
        self._bot.reply(cid, type, "[B][c]Đã vào team nhưng chưa quét được ID. Đang thử lại...")
        time.sleep(2.0)
        current_ids = list(set(self.ids)) if self.ids else []

    if not current_ids:
        self._bot.reply(cid, type, "[B][c]Lỗi: Không lấy được ID người trong team. Vui lòng thử lại!")
        self.sock39801.send(self._bot.leave_channel(uid, None))
        self.rstatus = (0, '')
        return

    # 3. Bắt đầu múa Full S7
    self._bot.reply(cid, type, f"[B][c]Đang múa Full S7 cho {len(current_ids)} người...")
    
    emotes = list(self.Emotes.values())
    random.shuffle(emotes)
    
    for emote in emotes:
        # Gửi lệnh múa cho danh sách ID đã lấy được
    ...
    self.sock39801.send(self._bot.leave_channel(uid, None))
    self.rstatus = (0, '') 
    return self._bot.reply(cid, type, "[B][c]HOÀN THÀNH: Đã mặc đồ, múa xong.")
