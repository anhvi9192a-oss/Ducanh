 ...
 def C1200(self, data, client): 
  try: 
   ...
   if message.startswith("/register"): 
     ...
     # nếu đã đăng ký rồi thì chặn vĩnh viễn 
     if UserRegister.is_registered(uid_int): 
 
          self._bot.reply(cid, type, """× Oops, Đăng ký thất bại 
 
Bạn đã đăng ký trước đó rồi. 
Mỗi UID chỉ được đăng ký dùng thử 1 lần duy nhất. 
 
Mua bot liên hệ: @cdanhdev""") 
     ...
     # đăng ký lần đầu 
     success, msg = UserRegister.register(uid_int, name) 
     ...
     if success: 
          ...
          # cấp quyền dùng 24h 
          self.manager.add_uid_to_access(botid, uid_int, "24h") 
...
/help để xem danh sách lệnh.""") 
          ...
          self._bot.reply(cid, type, "× Đăng ký thất bại!") 
          return False 
...
[ABB2BF]}}""".format(uid, name, exps)) 
    ...
    self._bot.reply(cid, type, """[B][c]-------------------------------------- 
/all s7 (bật full súng 7) 
/all rd s7 (random súng 7) 
/sk từ 1 đến 8 
...
--------------------------------------""")
    ...
    v = message.split() 
    if len(v) < 2: 
        return self._bot.reply(cid, type, "[B][c]Dùng: /all AK47 hoặc /all rd hoặc /all s7 hoặc /all rd s7") 
 
    if not self.ids: 
        return self._bot.reply(cid, type, "[B][c]Vui Lòng Dùng /js Trước") 
 
    cc = v[1].upper() 
    ids = list(set(self.ids)) 
    emotes = list(self.Emotes.values()) 
 
    if cc == "RD" and len(v) >= 3 and v[2].lower() == "s7": 
        emote_list = emotes[:] 
        random.shuffle(emote_list) 
        total_emotes = len(emote_list) 
        total_ids = len(ids) 
        index = 0 
        while index < total_emotes: 
            batch = emote_list[index:index + total_ids] 
 
            for i, uid in enumerate(ids): 
                if i < len(batch): 
                    self.sock39699.send( 
                        self._bot.play_emote(batch[i], [uid]) 
                    ) 
            index += total_ids 
            time.sleep(1.5) 
    ...
    if cc in self.Emotes: 
        ...
        return self._bot.reply(cid, type, "[B][c]Có sẵn:\n" + ", ".join(self.Emotes)) 
    ...
    uid_to_buff = msg.strip()

    self._bot.reply(
        cid,
        type,
        f"[B][c][00FFFF]Đang buff likes cho UID: [FFFFFF]{uid_to_buff}..."
    )

    try:
        api_likes = f"http://103.139.155.35:2100/likes?uid={uid_to_buff}&keys=ditconmemay"
        response = requests.get(api_likes, timeout=20)
        data = response.json()

        api_status = data["result"]["API"]["Success"]
        speed = data["result"]["API"]["speeds"]

        likes_added = data["result"]["Likes Info"]["Likes Added"]
        likes_before = data["result"]["Likes Info"]["Likes Before"]
        likes_after = data["result"]["Likes Info"]["Likes After"]

        nickname = data["result"]["User Info"]["Account Name"]
        level = data["result"]["User Info"]["Account Level"]
        region = data["result"]["User Info"]["Account Region"]

        # Nếu API báo thành công
        if api_status:
            success_msg = (
                "[B][c][00FF00]Buff like thành công!\n\n"
                f"[FFFFFF]Nickname: [00FFFF]{nickname}\n"
                f"[FFFFFF]Level: [FFFF00]{level}\n"
                f"[FFFFFF]Khu vực: [FFA500]{region}\n\n"
                f"[FFFFFF]Likes thêm: [00FF00]{likes_added}\n"
                f"[FFFFFF]Likes ban đầu: [FFA500]{likes_before}\n"
                f"[FFFFFF]Tổng số like: [00FF00]{likes_after}\n\n"
                f"[AAAAAA]Tốc độ xử lý: {speed}"
            )
        else:
            success_msg = (
                "[B][c][FFA500]Buff hoàn tất (chưa đủ số yêu cầu)\n\n"
                f"[FFFFFF]Nickname: [00FFFF]{nickname}\n"
                f"[FFFFFF]Level: [FFFF00]{level}\n\n"
                f"[FFFFFF]Likes thêm: [00FF00]{likes_added}\n"
                f"[FFFFFF]Likes ban đầu: [FFA500]{likes_before}\n"
                f"[FFFFFF]Tổng số like: [00FF00]{likes_after}\n\n"
                f"[FF0000]API Success: False\n"
                f"[AAAAAA]Tốc độ xử lý: {speed}"
            )
        ...
        self._bot.reply(cid, type, f"[FF0000]❌ Max Likes Hoặc Uid Saii!")
  ...
  try: 
   with self.config_lock: 
    ...
    for bot_id, bot_instance in self.bots.items(): 
     ...
     if hasattr(bot_instance, "botid"): 
      bot_config["botid"] = bot_instance.botid 
     if hasattr(bot_instance, "nickname"): 
