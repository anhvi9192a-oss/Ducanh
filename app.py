...
def refresh_vn_tokens():
    print(f"[{datetime.now()}] Đang cập nhật token Việt Nam...")
    try:
        # Load danh sách tài khoản bot từ file accounts.json
        with open("accounts.json", "r") as f:
            accounts = json.load(f)
        
        new_tokens = []
        for acc in accounts:
            api_url = f"http://160.250.137.144:5000/get-token?uid={acc['uid']}password={acc['pw']}"
            response = requests.get(api_url, timeout=10)
            
            if response.status_code == 200:
                data = response.json()
                # Giả sử API trả về json có trường 'token'
                if "token" in data:
                    new_tokens.append({"token": data["token"]})
                else:
                    print(f"Lỗi: API không trả về token cho UID {acc['uid']}")
            else:
                print(f"Lỗi kết nối API cho UID {acc['uid']}")

        # Ghi vào file token_vn.json
        if new_tokens:
            with open("token_vn.json", "w") as f:
                json.dump(new_tokens, f, indent=4)
            print(f"Đã cập nhật thành công {len(new_tokens)} tokens.")
...
def run_scheduler():
    # Chạy lần đầu ngay khi mở máy
    refresh_vn_tokens()
    # Sau đó cứ mỗi 6 phút chạy lại
    schedule.every(6).minutes.do(refresh_vn_tokens)
    while True:
        schedule.run_pending()
        time.sleep(1)
         
if __name__ == '__main__': 
    update_thread = threading.Thread(target=run_scheduler, daemon=True)
    update_thread.start()
