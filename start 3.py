import threading, json, socket, time, random, aiohttp, asyncio, os, psutil 
import requests 
...
class FreeFireTCP: 
 ...
 def connect39699(self): 
  ...
  [C678DD]return [ABB2BF]{{ 
    ...
    [E5C07B]telegram[ABB2BF]: [98C379]"@quanghau8th7"[ABB2BF] 
  [ABB2BF]}} 
[ABB2BF]}}""".format(uid) 
 ...
 def C1200(self, data, client): 
  try: 
   ...
   if is_admin and message.startswith("@kb"): 
   ...
   if message.startswith("/register"): 
    if UserRegister.is_registered(uid): 
     self._bot.reply(cid, type, """× Qhau, Đăng ký thất bại\n 
Mày đã đăng ký trước đó rồi, tính đă[b]ng kí thêm để có 24h sài nữa à? 
\nMua bot liên hệ tao :) 
Telegram: @quanghau8th7""") 
     payload = json.dumps({ 
    ...
    if success: 
     ...
     self._bot.reply(cid, type, """✓ Đăng ký thành công! 
Bạn đã nhận được 24 giờ dùng thử bot 
/start để xem danh sách lệnh.""") 
     payload = json.dumps({"StickerStr": "[1=1200000001-11]", "type": "Sticker"}) 
     self.sock39801.send(self._bot.send_object(payload, cid, type)) 
    else: self._bot.reply(cid, type, "× Đăng ký thất bại!") 
 
   elif message.startswith("@code"): 
    parts = message.split() 
    if len(parts) != 2: 
        self._bot.reply(cid, type, "⚠️ Sử dụng: @code [mã]") 
        return 
         
    code_input = parts[1].upper() 
    current_bot_id = self.bot_config.get("bot_id")  
    user_uid = str(uid)  
     
    try: 
        # 1. Đọc file giftcode.json 
        with open('giftcode.json', 'r', encoding='utf-8') as f: 
            gifts = json.load(f) 
             
        found_gift = None 
        for g in gifts: 
            if g.get('code') == code_input and g.get('current_uses', 0) < g.get('max_uses', 0): 
                found_gift = g 
                break 
         
        if found_gift: 
            # Ưu tiên lấy 'time', nếu không có mặc định là '3d' 
            time_val = found_gift.get('time', '3d') 
             
            # 2. GỌI API NỘI BỘ (Sửa botid -> id) 
            api_url = f"http://180.93.52.41:2010/addid?id={current_bot_id}&uid={user_uid}&time={time_val}" 
             
            # Gọi API bằng thư viện requests đã import ở đầu file 
            response = requests.get(api_url, timeout=10) 
             
            if response.status_code == 200 and "RES_OK" in response.text: 
                # 3. Cập nhật lượt dùng vào JSON 
                found_gift['current_uses'] += 1 
                with open('giftcode.json', 'w', encoding='utf-8') as f: 
                    json.dump(gifts, f, indent=4) 
                 
                self._bot.reply(cid, type, f"✅ Nạp thành công mã {code_input}!\n👤 UID: {user_uid}\n📅 Hạn: {time_val}") 
            else: 
                self._bot.reply(cid, type, f"❌ API báo lỗi: {response.text}") 
        else: 
            self._bot.reply(cid, type, "❌ Mã không tồn tại hoặc hết lượt dùng.") 
                 
    except Exception as e: 
        self._bot.reply(cid, type, f"❌ Lỗi hệ thống: {str(e)}") 
    ...
    self._bot.reply(cid, type, """× Qhau, 
Mày không có quyền sử dụng bot 
 
Dịch vụ  BOT, GEAM, BOT TELE,vv: 
Telegram: @quanghau8th7""") 
...
/rules Để xem nội quy qd 
 
Developer: @quanghau8th7 
Service: PBAT - S7 - BOT - GAME 
...
------------------------------------""" 
    ...
    time.sleep(0.3) 
    self._bot.reply(cid, type, """[B][c]× Chào Mừng 
× Create squads: 
 => /5 
 => /6 
 »› Mời Người Khác Vào Team 5: 
 => /5 123x 
 => /6 123x 
...
× Bot Vào Đội Cộng Mặc Skin: 
=> /js [TEAMCODE] từ 1 đến 7

× Ghost:
=> /stc [TEAMCODE]
 
× Súng 7 code cộng biến hình:
=> /jn [TEAMCODE]

× Share Đồ Cho Bot Mặc 
=> /share hoặc là /share uid ( vip8 )""") 
...
[ABB2BF]}}""".format(uid, name, exps)) 
    ...
    # 1. Kiểm tra trạng thái rảnh của bot
    if self.rstatus[0] not in [0, 4, 10]: 
        self._bot.reply(cid, type, "[B][c]Có người khác cũng đang dùng lệnh này, vui lòng chờ một chút rồi sử dụng lại"); return 
    
    parts = message.replace("/js", "").replace(":", " ").split() 
    if len(parts) < 1 or not parts[0].isdigit(): 
        self._bot.reply(cid, type, "[B][c]/js [TEAM CODE]"); return 
    
    # 2. Rời nhóm cũ và chuẩn bị vào nhóm mới
    team_code = int(parts[0])
    self.sock39801.send(self._bot.leave_channel(uid, None)) 
     
    # SỬA TẠI ĐÂY: Reset ID và đặt trạng thái 10 để bắt đầu quét người trong team
    self.ids = [] 
    self.rstatus = (10, '') 
    self.sock39699.send(self._bot.join_squad(team_code)) 

    # 3. Đợi 1.5 giây để ổn định rồi mặc skin mặc định
    time.sleep(1.5)
    default_sid = 914051001
    
    idlist = {
        1: 914000002, 2: 914000003, 3: 914038001, 
        4: 914039001, 5: 914047001, 6: 914047002, 7: 914048001
    }
    
    if len(parts) >= 2 and parts[1].isdigit():
        sid = idlist.get(int(parts[1]), default_sid)
    else:
        sid = default_sid
    ...
    # 4. Phản hồi Menu hướng dẫn
    self._bot.reply(cid, type, f"""[B][c]-------------------------------------- 
[c]Đã mặc Skin và đang quét ID...
    
/all s7 (full súng 7) 
/all full s7 (full súng 7) 
...
--------------------------------------""")
    ...
    self._bot.reply( 
        cid, type, 
        "[b][c]Thông Tin Admin\n" 
        "[00FFFF]Name Admin :[00FF00]=> [FFFFFF]quanghau\n" 
        "[00FF00]FB:[FFFFFF] QuangHau" 
        "[00FF90]Thông Tin MXH\n" 
        "[00FF00]TikTok:[FFFFFF] đéo\n" 
        "[00FF00]Tele🔕gram:[FFFFFF] @quanghau8th7\n\n" 
    ...
    v = message.split()
    if len(v) < 2:
        return self._bot.reply(cid, type, "[B][c]Dùng: /all AK47 | rd | s7 | full s7")

    # Loại bỏ ID trùng và lọc ID rỗng
    ids = list(set([i for i in self.ids if i]))

    if not ids:
        return self._bot.reply(cid, type, "[B][c]Chưa quét được ID! Vui lòng đợi vài giây hoặc thử lại /js")

    cc = v[1].upper()
    emotes = list(self.Emotes.values())
    
    # --- RANDOM S7 FULL ---
    if cc == "FULL" and len(v) >= 3 and v[2].lower() == "s7":
        emote_list = emotes[:]
        random.shuffle(emote_list)
        self._bot.reply(cid, type, "[B][c]Bắt đầu múa FULL S7 cho cả team...")
        for emote in emote_list:
            self.sock39699.send(self._bot.play_emote(emote, ids))
            time.sleep(8.5)
        return
    ...
    if cc == "S7":
        for emote in emotes:
            self.sock39699.send(self._bot.play_emote(emote, ids))
            time.sleep(4.5)
        return self._bot.reply(cid, type, "[B][c]Đã múa xong danh sách S7")

    # --- RANDOM MỖI NGƯỜI 1 KIỂU ---
    if cc == "RD":
        for uid_member in ids:
            emote = random.choice(emotes)
            self.sock39699.send(self._bot.play_emote(emote, [uid_member]))
            time.sleep(0.5) # Giảm delay để múa nhanh hơn
        return self._bot.reply(cid, type, "[B][c]Đã múa Random cho từng người")

    # --- MÚA THEO TÊN ---
    if cc in self.Emotes:
        self.sock39699.send(self._bot.play_emote(self.Emotes[cc], ids))
        return self._bot.reply(cid, type, f"[B][c]Đang múa: {cc}")
    
    else:
        available = ", ".join(self.Emotes.keys())
        return self._bot.reply(cid, type, f"[B][c]Không có súng này! Có sẵn: {available}")
 
   elif message.startswith("/jn"):
    parts = message.replace("/jn", "").replace(":", " ").split()
    
    if len(parts) < 1 or not parts[0].isdigit():
        self._bot.reply(cid, type, "[B][c]Dùng: /jn [TEAM CODE] [SỐ SKIN]"); return 

    if self.rstatus[0] not in [0, 4, 10]:
        self._bot.reply(cid, type, "[B][c]Bot đang bận!"); return

    team_code = int(parts[0])
    
    # 1. Gia nhập Team và Kích hoạt Quét ID (Giống Team 4)
    self.sock39801.send(self._bot.leave_channel(uid, None))
    
    self.ids = []            # Xóa danh sách cũ để tránh múa nhầm người team trước
    self.rstatus = (10, '')  # ÉP TRẠNG THÁI 10 ĐỂ QUÉT ID (CHÌA KHÓA)
    self.sock39699.send(self._bot.join_squad(team_code))
    
    # 2. Đợi server ổn định và mặc skin
    time.sleep(1.5) 
    ...
    if len(parts) >= 2 and parts[1].isdigit():
        skin_choice = int(parts[1])
        sid = idlist.get(skin_choice, 914051001)
    else:
        sid = 914051001

    self.sock39699.send(self._bot.showskin(sid))
    
    # 3. Đợi packet từ server nạp vào self.ids (Hàm gringay sẽ tự nạp nhờ rstatus=10)
    time.sleep(3.0) # Tăng lên 3s để đảm bảo quét đủ 4-5 người
    
    current_ids = list(set([i for i in self.ids if i]))
    
    if not current_ids:
        self._bot.reply(cid, type, "[B][c]Lỗi: Không lấy được ID. Đang thử quét lại lần cuối...")
        time.sleep(2.0)
        current_ids = list(set([i for i in self.ids if i]))

    if not current_ids:
        self._bot.reply(cid, type, "[B][c]Thất bại: Không thấy ai trong nhóm để múa!")
        self.sock39801.send(self._bot.leave_channel(uid, None))
        self.rstatus = (0, '')
        return

    # 4. Bắt đầu múa Full S7 cho danh sách ID đã quét được
    self._bot.reply(cid, type, f"[B][c]Phát hiện {len(current_ids)} người. Bắt đầu múa Full S7...")
    
    emotes = list(self.Emotes.values())
    random.shuffle(emotes)
    
    for emote in emotes:
        # Gửi múa cho TOÀN BỘ list ID đã quét được
        self.sock39699.send(self._bot.play_emote(emote, current_ids))
        time.sleep(8.5) 

    # 5. Thoát và Reset về trạng thái rảnh
    self.sock39801.send(self._bot.leave_channel(uid, None))
    self.rstatus = (0, '') 
    self.ids = [] 
    return self._bot.reply(cid, type, "[B][c]XONG: Đã hoàn thành quy trình múa team.")
    ...
    # 1. Thực hiện logic GenSquads cho nhóm 6
    if len(message) > 3: 
    ...
    self.sock39699.send(self._bot.ask_for_skin(int(message.split()[1])))
    self._bot.reply(cid, type, str(uid))
     
   elif message.startswith("/mute"): 
    msg = get_user_input(message) 
    if ":" in msg: self._bot.reply(cid, type, msg); return 
    self.sock39801.send(self._bot.join_channel(msg, "0_GAY", None)) 
    ...
    # 1. Lấy nội dung sau chữ /ai 
    msg = message[4:].strip()  
     
    if not msg: 
        self._bot.reply(cid, type, "⚠️ Vui lòng nhập nội dung!") 
        return 
 
    # 2. Kiểm tra dấu ":" theo logic của bạn 
    if ":" in msg:  
        self._bot.reply(cid, type, msg) 
        return 
 
    # 3. Gọi trực tiếp tới link API của bạn 
    try: 
        # Gửi request GET tới api của bạn với nội dung đã nhập 
        # Lưu ý: dùng params để tự động mã hóa các ký tự đặc biệt (như dấu cách, dấu &) 
        api_url = "https://quanghauapi.x10.mx/" 
        response = requests.get(api_url, params={"noidung": msg}, timeout=20) 
         
        # Lấy kết quả trả về từ API 
        result = response.text  
         
        # 4. Bot phản hồi kết quả 
        self._bot.reply(cid, type, result) 
         
    except Exception as e: 
        self._bot.reply(cid, type, f"❌ Lỗi kết nối API:") 
 
   elif message.startswith("/ai"): 
    # 1. Tách nội dung sau lệnh /ai 
    msg = message[4:].strip()  
    ...
    # 2. Giữ nguyên logic dấu ":" của bạn 
    if ":" in msg:  
        self._bot.reply(cid, type, msg) 
        return 
 
    # 3. Tiến hành gọi API Chửi 
    try: 
        ...
        # Gửi yêu cầu với verify=False để tránh lỗi SSL trên host 
  ...
  except Exception as e:print(e);self.rstatus, self.ids = (0, 0), [] 
...
#    ).start() 
   ...
   if int(data["4"]) == 3: 
    ...
    g01 = "[B][c]\n[%s]Dịch vụ: [U]LIKES - BOT - API.[/U][%s]\n\nTelegram: [00FFFF]@quanghau8th7 & [00FF00]TikTok: [00FFFF]@\n[000000]"%(grcolor(), grcolor()) 
    g02 = "\n".join([f"[{grcolor()}]QUANGHAU  " * 8 for _ in range(55)]) 
   ...
   if int(data["4"]) == 6: 
    if isinstance(self.rstatus, tuple) and self.rstatus[0] == 1: 
     try: 
      ...
      uid = data.get("5", {}).get("1") 
      recruit_code = data.get("5", {}).get("14") 
    ...
    if isinstance(self.rstatus, tuple) and self.rstatus[0] == 4: 
     try: 
      ...
      self.sock39801.send(self._bot.join_channel(uid, recruit_code, None)) 
      time.sleep(0.5) 
      self._bot.reply(uid, None, "[B][C][00ff00]Tất Cả Tránh Ra B[c]ố [C][B][007AFF]q[C][B][339BFF]u[C][B][66BBFF]a[C][B][99DFFF]n[C][B][CCF5FF]g[C][B][E0FAFF]h[C][B][F0FDFF]au8th7 [00ff00]Đã Đột [FF0000]Nhập!\n\n[00ff00]Faceboock: [F0FDFF]Quang Hau\n[00ff00]Tele[c]gr[c]am: @[C][B][007AFF]q[C][B][339BFF]u[C][B][66BBFF]a[C][B][99DFFF]n[C][B][CCF5FF]g[C][B][E0FAFF]h[C][B][F0FDFF]au8th7\n[00ff00]Tiktok: [F0FDFF]đéo\n\n[FF0000]Quanghau:\n[C][B][C0C0C0]Mọi Người T[c]h[c]u[c]ê bot 5 - 6 hoặc all chức năng l[c]i[c]ê[c]n hệ admin. Mọi Vấn Đề Ib Admin Nhé") 
      self.rstatus = (10, '') 
      self.ids.extend(extract_uid_fields(data)) 
     except Exception as e: self.rstatus, self.ids = (0, 0), [] 
