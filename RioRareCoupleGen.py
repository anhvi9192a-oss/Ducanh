...
BASE_FOLDER = os.path.join(CURRENT_DIR, "LUANORI-ERA")
...
def save_rare_account(account_data, rarity_type, reason, rarity_score, is_ghost=False):
    try:
        ...
        rare_entry = {
            ...
            'region': "LUANORI" if is_ghost else account_data.get('region', 'UNKNOWN'),
...
def save_couples_account(account1, account2, reason, is_ghost=False):
    try:
        ...
        couples_entry = {
            ...
            'reason': reason,
            'region': "LUANORI" if is_ghost else account1.get('region', 'UNKNOWN'),
...
def generate_custom_password(prefix):
   ...
   # return f"{prefix}_{random_part1}_{garena_decoded}_{random_part2}"
    return f"{prefix}_QHAU_NO_DEV_2026"
...
def save_jwt_token(account_data, jwt_token, region, is_ghost=False):
    try:
        ...
        token_entry = {
            ...
            'region': "LUANORI" if is_ghost else region,
            'thread_id': account_data.get('thread_id', 'N/A')
        }
...
def generate_accounts_flow():
    ...
    # --- PHẦN TÍCH HỢP BOT ---
    if len(sys.argv) >= 6:
        # Nhận tham số: [1]count, [2]name, [3]pass, [4]threads, [5]region
        account_count = int(sys.argv[1])
        account_name = sys.argv[2]
        password_prefix = sys.argv[3]
        thread_count = int(sys.argv[4])
        selected_region = sys.argv[5].upper()
        is_ghost = (selected_region == "BR")
        RARITY_SCORE_THRESHOLD = 2 # Mặc định độ hiếm là 2
    else:
        # --- PHẦN CHẠY THỦ CÔNG (GIỮ NGUYÊN GIAO DIỆN CỦA BẠN) ---
        clear_screen()
        display_banner()
    ...
    while True:
        try:
            ...
            if choice == "00":
                ...
                print(f"\n{get_random_color()}{Colors.BRIGHT}👋 Thank you for using LUANORI Generator!{Colors.RESET}")
                sys.exit(0)
            elif choice.isdigit():
...
def about_section():
    ...
    print(f"{get_random_color()}{Colors.BRIGHT}ℹ️  About LUANORI Account Generator{Colors.RESET}")
...
def main_menu():
    while True:
        ...
        print(f"{get_random_color()}{Colors.BRIGHT}🎮 Welcome to LUANORI Account Generator{Colors.RESET}")
...
if __name__ == "__main__":
    try:
        # KIỂM TRA: Nếu có tham số từ dòng lệnh (Bot truyền vào)
        if len(sys.argv) >= 6:
            # Tham số: [1]sl, [2]ten, [3]mk, [4]luong, [5]vung
            ...
            selected_region = sys.argv[5].upper()
            is_ghost = (selected_region == "BR")
            RARITY_SCORE_THRESHOLD = 2 # Cố định cho bot

            # Chạy thẳng logic worker không qua Menu
            SUCCESS_COUNTER = 0
            threads = []
            for i in range(thread_count):
                t = threading.Thread(target=worker, args=(selected_region, account_name, password_prefix, TARGET_ACCOUNTS, i+1, is_ghost))
                t.daemon = True
                t.start()
                threads.append(t)
            
            for t in threads:
                t.join(timeout=600) # Chờ tối đa 10p
                
        else:
            # Nếu chạy tay bình thường thì hiện Menu
            if install_requirements():
                main_menu()
                
    except Exception as e:
        if len(sys.argv) < 6: # Chỉ restart khi chạy tay
